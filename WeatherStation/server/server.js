...
app.get("/getLatest", function (req, res) {
  (async function() {
    ...
    let from = parseInt(req.query.from); // 'gt' value
    let to = parseInt(req.query.to);     // 'lt' value

    try {
      let result = await db.collection("data")
                           .find({ time: { $gt: from, $lt: to } })  // Filters for the specified range
                           .sort({ time: -1 })
                           .toArray();
